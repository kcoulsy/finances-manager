...
export function ImportTransactionsDialog() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (result?.data?.success) {
        ...
        if (result.data.accounts.length > 0 && !selectedAccountId) {
  ...
  const parseCSV = (
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const number =
        numberIndex !== -1 ? values[numberIndex]?.trim() : undefined;
      const subcategory =
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // If no account number detected or account selection required
      if (!selectedAccountId) {
        alert("Please select an account or create one from the CSV.");
        return;
      }
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file with your transaction data. Expected columns:
